...
$company_stamp = 'assets/images/stamp-placeholder.png'; // Default fallback
$debug_messages = [];
try {
    ...
    if ($stamp_path && file_exists($full_path)) {
        ...
        error_log("Stamp found: $company_stamp");
    } else {
        $debug_messages[] = "Stamp not found: path=" . ($stamp_path ?? 'none') . ", exists=" . ($full_path && file_exists($full_path) ? 'yes' : 'no');
        error_log("Stamp not found: path=" . ($stamp_path ?? 'none') . ", exists=" . ($full_path && file_exists($full_path) ? 'yes' : 'no'));
    }
} catch (PDOException $e) {
    $debug_messages[] = "Stamp query failed: " . $e->getMessage();
    error_log("Stamp query failed: " . $e->getMessage());
}
...
error_log("Land ID $land_id status: $status");
...
澜        </div>
    ...
    <script>
        // Ensure the page prints properly
