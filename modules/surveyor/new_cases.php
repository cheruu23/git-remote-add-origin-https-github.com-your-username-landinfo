...
// Check if user is a manager, record officer, or surveyor
if (!isset($_SESSION['user']['role']) || !in_array($_SESSION['user']['role'], ['manager', 'record_officer', 'surveyor'])) {
    $_SESSION['error'] = $translations[$lang]['access_denied'] ?? 'Access denied! Only managers, record officers, and surveyors can view new cases.';
...
try {
    ...
    $sql = "SELECT id, title, description, case_type, land_id, created_at, investigation_status 
            FROM cases 
            WHERE viewed = 0 AND assigned_to = ? 
    ...
    $stmt->close();
    logAction('view_new_cases', 'User viewed new cases', 'info');

    // Mark cases as viewed
    if (!empty($case_ids)) {
        $placeholders = implode(',', array_fill(0, count($case_ids), '?'));
        $sql = "UPDATE cases SET viewed = 1 WHERE id IN ($placeholders)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param(str_repeat('i', count($case_ids)), ...$case_ids);
        $stmt->execute();
        $stmt->close();
        logAction('mark_cases_viewed', 'Marked cases as viewed: ' . implode(', ', $case_ids), 'info');
    }
} catch (Exception $e) {
    logAction('fetch_cases_failed', 'Failed to fetch new cases: ' . $e->getMessage(), 'error');
}
...
<head>
    ...
    <title><?php echo $translations[$lang]['new_cases_title']; ?> - LIMS</title>
...
<body>
    ...
    <div class="content" id="main-content">
        <div class="container mt-3">
            <h2 class="text-center"><?php echo $translations[$lang]['new_cases_title']; ?></h2>
            <div class="card">
                <div class="card-body p-0">
                    ...
                    <?php endif; ?>
                    <?php if (empty($cases)): ?>
                        <div class="alert alert-info"><?php echo $translations[$lang]['no_new_cases']; ?></div>
                    <?php else: ?>
                                ...
                                <tbody>
                                    <?php foreach ($cases as $case): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="<?php echo BASE_URL; ?>/modules/surveyor/view_case.php?case_id=<?php echo $case['id']; ?>&lang=<?php echo $lang; ?>" 
                                                   class="btn btn-primary">
                                                    <i class="fas fa-eye"></i> <?php echo $translations[$lang]['view_case'] ?? 'View Case'; ?>
                                                </a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
