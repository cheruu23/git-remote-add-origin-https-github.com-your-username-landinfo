                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($row['status'] === 'Pending') : ?>
                                <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#completeCaseModal<?= $row['id']; ?>">
                                    Mark as Completed
                                </button>

                                <!-- Modal for Completing Case -->
                                <div class="modal fade" id="completeCaseModal<?= $row['id']; ?>" tabindex="-1" aria-labelledby="completeCaseLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title">Complete Case #<?= $row['id']; ?></h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <form action="complete_case.php" method="post">
                                                <div class="modal-body">
                                                    <input type="hidden" name="case_id" value="<?= $row['id']; ?>">
                                                    <label for="survey_notes" class="form-label">Surveyor Notes:</label>
                                                    <textarea class="form-control" name="survey_notes" rows="3" required></textarea>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                                    <button type="submit" class="btn btn-success">Complete Case</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
